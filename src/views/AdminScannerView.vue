...
const onBarcodeDetected = async (ean) => {
  ...
  try {
    ...
    if (data.ingredientes || data.trazas || data.puede_contener) {
      ...
      trazasText.value = (data.trazas ?? data.puede_contener ?? '')
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean)
        .join(', ');
...
const extractIngredients = async () => {
  ...
  try {
    ...
    applyIngredientItems(result.ingredientes);
    step.value = 'review';
    fetchSimilarProducts();
  } catch (err) {
...
const saveIngredients = async () => {
  const parsed = [
    ...parseIngredientText(ingredientesText.value, false),
    ...parseIngredientText(trazasText.value, true),
  ];
...
function onOcrDone(ingredientes) {
  applyIngredientItems(ingredientes);
  step.value = 'review';
}

// ─── Lifecycle ───────────────────────────────────────────────────────────────
function normalizeOcrIngredient(item) {
  if (item && typeof item === 'object') {
    return {
      nombre: String(item.nombre ?? item.name ?? item.ingredient ?? '').trim(),
      is_trace: item.is_trace === true || item.is_trace === 1 || item.trace === true || item.isTrace === true,
    };
  }
...
function applyIngredientItems(items) {
  const normalized = (items ?? [])
    .map(normalizeOcrIngredient)
    .filter((item) => item.nombre);

  const declared = normalized.filter((item) => !item.is_trace).map((item) => item.nombre);
  const traces = normalized.filter((item) => item.is_trace).map((item) => item.nombre);

  ingredientsList.value = declared;
  ingredientesText.value = declared.join(', ');
  trazasText.value = traces.join(', ');
}

function parseIngredientText(text, isTrace) {
  return text
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
    .map((nombre) => ({ nombre, is_trace: isTrace }));
}
      ...
      <template v-else-if="step === 'review'">
        <div class="px-4 py-6 space-y-4">
          ...
          <div class="bg-white rounded-2xl shadow p-4 space-y-3">
            ...
            <div class="space-y-1">
              <p class="font-semibold text-gray-700 text-sm">Puede contener / trazas</p>
              <textarea
                v-model="trazasText"
                rows="4"
                class="w-full border border-amber-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 resize-none"
                placeholder="mani, leche, soja..."
              ></textarea>
            </div>
